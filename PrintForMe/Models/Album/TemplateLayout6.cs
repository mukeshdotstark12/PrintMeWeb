using System;

namespace PrintForMe.Models.Album
{
    public class TemplateLayout6
    {
        public Guid AlbumPageRowGUID { get; set; }
        public string PageLayoutCode { get; set; }
        public int PhotoCount { get; set; }
        public string Name_1 { get; set; }
        public int? Sequence_1 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_1 { get; set; }
        public string Name_2 { get; set; }
        public int? Sequence_2 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_2 { get; set; }
        public string Name_3 { get; set; }
        public int? Sequence_3 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_3 { get; set; }
        public string Name_4 { get; set; }
        public int? Sequence_4 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_4 { get; set; }
        public string Name_5 { get; set; }
        public int? Sequence_5 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_5 { get; set; }
        public string Name_6 { get; set; }
        public int? Sequence_6 { get; set; }
        public Guid? AlbumPagePhotoRowGUID_6 { get; set; }
    }
}